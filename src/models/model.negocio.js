...
function Inserir(json_neg, callback) {
    ...
    let ssql = "INSERT INTO negocio(id_usuario, etapa, descricao, empresa, ";
...
function Editar(id_negocio, json_neg, callback) {
    ...
    let ssql = "UPDATE negocio SET etapa= ?, descricao= ?, ";
...
function Excluir(id_negocio, callback) {
    ...
    db.getConnection(function(err, conn){
        conn.beginTransaction(async function(err){
            try {
                let ssql = "DELETE FROM negocio_tarefa WHERE id_negocio = ?";
                await executeQuery(conn, ssql, [id_negocio]);
                console.log(executeQuery);

                ssql = "DELETE FROM negocio WHERE id_negocio = ?";
                await executeQuery(conn, ssql, [id_negocio]);
